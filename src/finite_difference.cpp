...
Hessian FiniteDifference::compute_hessian(
    ...
    const double ang_to_bohr_sq = ang_to_bohr * ang_to_bohr;
    ...
    for (int i = 0; i < size; i++) {
        for (int j = i; j < size; j++) {
            ...
            if (i == j) {
                ...
                mol_plus.set_coordinate(
                    i,
                    mol_plus.get_coordinate(i) + step_size
                );

                mol_minus.set_coordinate(
                    i,
                    mol_minus.get_coordinate(i) - step_size
                );

                double e_plus = engine.compute_energy(mol_plus);
                double e_minus = engine.compute_energy(mol_minus);

                double raw = (e_plus - 2.0 * e0 + e_minus) /
                             (step_size * step_size);

                value = raw / ang_to_bohr_sq;
            } else {
                ...
                double raw = (e_pp - e_pm - e_mp + e_mm) /
